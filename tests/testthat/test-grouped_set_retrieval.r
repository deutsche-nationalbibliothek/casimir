test_that("grouping (doc-strata) of set retrieval computation works", {
  ...
  compare_wo_explicit_strata <- casimir:::create_comparison(gold, pred, doc_strata = NULL)
  expect_equal(
  ...
  if (Sys.getenv("LANG") == "en_US.UTF-8") {
    expect_error(
      compute_set_retrieval_scores_dplyr_version(gold, pred, mode = "doc-avg", doc_strata = "false_col"),
      regexp = "doc_strata %in% colnames\\(gold_standard\\) is not TRUE"
    )
    expect_error(
      compute_set_retrieval_scores(gold, pred, mode = "doc-avg", doc_strata = "false_col"),
      regexp = "doc_strata %in% colnames\\(gold_standard\\) is not TRUE"
    )
  }

  res <- compute_set_retrieval_scores_dplyr_version(gold, pred, mode = "doc-avg", doc_strata = "hsg") # , grouping_var = "hsg"
  res_collapse <- compute_set_retrieval_scores(gold, pred, mode = "doc-avg", doc_strata = "hsg")

  res_001 <- compute_set_retrieval_scores_dplyr_version(gold_001, pred_001, mode = "doc-avg")
  res_002 <- compute_set_retrieval_scores_dplyr_version(gold_002, pred_002, mode = "doc-avg")
  ...
  expect_warning(
    object = compute_set_retrieval_scores_dplyr_version(gold, pred,
                                          mode = "doc-avg",
                                          doc_strata = "hsg",
                                          compute_bootstrap_ci = TRUE,
                                          n_bt = 10L, seed = 134),
    regexp = "hsg is not a factor variable. Some levels may be lost in bootstrap replications"
  )
  ...
  compare <- create_comparison(gold_standard = gold_w_factor,
                               predicted = pred,
                               doc_strata = "hsg")
  boot_res <- generate_replicate_results_dplyr_version(base_compare = compare,
                                         n_bt = 10L,
                                         seed = 134,
  ...
  res_across_config <- purrr::map2(
    ...
    .f = ~ expect_silent(
      object = compute_set_retrieval_scores_dplyr_version(
        gold_w_factor,
        pred,
        mode = .x,
        doc_strata = "hsg",
...
test_that("grouping (label-strata) of set retrieval computation works", {
  ...
  # expect no error when adding strata
  expect_silent(casimir:::create_comparison(gold, pred, label_dict = label_dict))

  # check if invalid colname in label dictionary is rejected
  expect_error(
    compute_set_retrieval_scores_dplyr_version(gold, pred, mode = "doc-avg",
                                 label_dict = dplyr::rename(label_dict, gnd_idn = label_id)),
    regexp = "\"label_id\" %in% colnames\\(label_dict\\) is not TRUE"
  )
  expect_error(
    compute_set_retrieval_scores(gold, pred, mode = "doc-avg",
                                 label_dict = dplyr::rename(label_dict, gnd_idn = label_id)),
    regexp = "\"label_id\" %in% colnames\\(label_dict\\) is not TRUE"
  )
  ...
  ####################################################
  comparison_w_label_groups <- casimir:::create_comparison(gold, pred, label_dict = label_dict)
  interm_res_grpd_act <- dplyr::select(
    compute_intermediate_results_dplyr_version(comparison_w_label_groups, rlang::syms(c("gnd_entity"))),
    "gnd_entity", "tp", "fn", "fp"
  )
  interm_res_grpd_act_collapse <- dplyr::select(
    compute_intermediate_results(
      comparison_w_label_groups,
      c("gnd_entity"))$results_table,
    "gnd_entity", "tp", "fn", "fp"
  )
  ...
  res_collapse <- purrr::imap(
    ...
    .f = ~expect_equal(
      object = compute_set_retrieval_scores(gold, pred,
                                            mode = .y,
                                            label_dict = label_dict),
      expected = .x)
  )
  ...
  expect_warning(
    ...
    regexp = "gnd_entity is not a factor variable. Some levels may be lost in bootstrap replications")

  label_dict_w_factor <- dplyr::mutate(label_dict, gnd_entity = as.factor(gnd_entity))
  compare <- create_comparison(gold, pred, label_dict = label_dict_w_factor)
  boot_res <- generate_replicate_results_dplyr_version(
  ...
  messages <- purrr::pmap(
    configuration,
    .f = function(.mode, .compute_bootstrap_ci, ..progress) {
      expect_silent(
        object = compute_set_retrieval_scores_dplyr_version(
          gold,
          pred,
