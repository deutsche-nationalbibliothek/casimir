...
#'
#' @param gold_standard expects \code{data.frame} with cols \emph{"label_id", "doc_id"}
...
#'
#' @return a \code{data.frame} with cols \emph{"metric", "mode", "value", "support"}
...
#'   \item{\code{mode = "subj-avg"}}{the number of labels contributing to the subj-average}
...
compute_set_retrieval_scores <- function(
  ...
  test_factor <- function(var) {
    if (!is.factor(compare[[var]]))
      warning(
        paste(
          var,
          "is not a factor variable. Some levels may be lost in bootstrap replications"))
  }
  ...
  if (compute_bootstrap_ci == FALSE) {
    ...
    smry_grouping_var <- c("metric", setdiff(grouping_var, c("doc_id", "label_id")))
...
compute_set_retrieval_scores_dplyr_version <- function(
    gold_standard,
    predicted,
    mode = "doc-avg",
    compute_bootstrap_ci = FALSE,
  ...
  if (compute_bootstrap_ci == FALSE) {
    ...
    intermediate_results <- compute_intermediate_results_dplyr_version(
      compare,
      rlang::syms(grouping_var),
      propensity_scored = ps_flags$intermed,
      cost_fp = cost_fp_processed
      )
    results <- summarise_intermediate_results_dplyr_version(
      intermediate_results,
      propensity_scored = ps_flags$summarise,
      label_distribution = label_distribution
      )
    ...
    boot_results <- generate_replicate_results_dplyr_version(
      base_compare = compare,
      n_bt = n_bt,
      grouping_var = grouping_var,
      seed = seed,
